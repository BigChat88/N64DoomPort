...
void W_AddFile(char *filename)
{
    ...
    // Unconditional (not RANGECHECK-gated, unlike the rest of this
    // function): RANGECHECK is off in the shipped build (see Makefile), so
    // this used to be the one failure this function could silently ignore -
    // header would be read from an invalid handle, and header.numlumps
    // (garbage) would flow straight into the alloca() below.
    if (-1 == handle)
    {
    ...
    for (i=startlump ; i<numlumps ; i++,lump_p++, fileinfo++)
    {
        ...
        // name[] is 9 bytes (w_wad.h) for exactly this reason, but strncpy
        // only guarantees a terminator when the source is shorter than 8
        // bytes - an on-disk name that fills all 8 (common) leaves this
        // uninitialized (lumpinfo comes from realloc, which doesn't zero).
        // Every in-tree comparison (comp_keys/wadupr/W_LumpNameHash) is
        // already hard-bounded to 8 chars regardless, so this has never
        // been reachable as a real bug here - only closing it off in case
        // something later prints/compares this field as a plain C string.
        lump_p->name[8] = '\0';
...
int W_CheckNumForName (char* name)
{
    ...
    testlump->name[8] = '\0'; // see the same fix in W_AddFile for why
