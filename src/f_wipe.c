...
// wipe_scr gets pointed to actual screen (8bpp palettized framebuffer)
extern void*    bufptr;
...
int wipe_initMelt(int width, int height, int ticks)
{
    ...
    // (screen is 8bpp, so 4 pixels pack into each uint32_t unit below)
    wipe_shittyColMajorXform((uint32_t*)wipe_scr_start, width/4, height);
    wipe_shittyColMajorXform((uint32_t*)wipe_scr_end, width/4, height);
