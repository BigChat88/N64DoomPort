...
void node_reorder(int numn, node_t *np)
{
    ...
    // rewrites is allocated with exactly numn elements (line 82) and every
    // access below is guarded by "< numn", so this only needs to clear numn
    // elements - the (numn+1) here zeroed 2 bytes past the allocation on
    // every level load.
    memset(rewrites,  0, numn*sizeof(uint16_t));
