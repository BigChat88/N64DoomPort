...
// I_Tactile is implemented in i_input.c (Rumble Pak control lives with the
// rest of the joypad code there).
...
byte* I_ZoneBase(int* size)
{
    ...
    *size = zone_size;
    ...
    // Z_Init (the only caller) writes through this pointer unconditionally
    // right after calling this - unlike every allocator built on top of it
    // (Z_Malloc et al. all call I_Error on failure), a failed malloc here
    // would otherwise be a silent NULL-pointer write with no diagnostic.
    if (!ptr)
    {
        I_Error("I_ZoneBase: failed to allocate %d bytes for the zone", *size);
    }
...
{   
    ...
    printf("I_Error: %s\n", errstr);
#if 1
    // lockVideo(1) calls libdragon's display_get(), which asserts if
    // display_init() was never called - reachable here, since I_Error can
    // fire earlier than I_InitGraphics (e.g. a bad WAD file, checked well
    // before I_Init() runs in D_DoomMain). Skip the on-screen error box in
    // that case; the printf above is still there either way.
    if (display_ready)
    {
        unlockVideo(_dc);
        for(int i=0;i<2;i++)
